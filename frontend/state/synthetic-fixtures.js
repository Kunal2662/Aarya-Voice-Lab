...
// Shaped like aarya_voice_lab.core.capability.Capability.to_dict() --
// the same closed vocabulary environment.audit.run_audit() produces.
// Deliberately a CPU-only-looking profile: no GPU vendor is assumed or
// privileged, matching the project's hardware-agnostic requirement.
export function syntheticHardwareCapabilities() {
  return [
    { name: "Python", state: "AVAILABLE", detail: "recommended version", version: "3.12.4" },
    { name: "pip", state: "AVAILABLE", detail: "", version: "24.0" },
    { name: "virtualenv support", state: "AVAILABLE", detail: "stdlib venv importable", version: null },
    { name: "CPU", state: "AVAILABLE", detail: "4 physical / 8 logical", version: null },
    { name: "RAM", state: "AVAILABLE", detail: "16.0 GB total", version: null },
    { name: "Disk space", state: "AVAILABLE", detail: "120.0 GB free", version: null },
    { name: "FFmpeg", state: "AVAILABLE", detail: "", version: "6.1" },
    { name: "NVIDIA GPU", state: "OPTIONAL", detail: "no NVIDIA GPU detected; CPU-only execution is supported (slower)", version: null },
    {
      name: "Accelerator (any vendor)",
      state: "OPTIONAL",
      detail: "no NVIDIA, AMD, or PCI-enumerable accelerator detected; CPU-only execution is supported (slower)",
      version: null,
    },
    { name: "CUDA runtime", state: "UNKNOWN", detail: "torch is not installed, so CUDA availability cannot be confirmed here", version: null },
    { name: "CUDA toolkit (nvcc)", state: "OPTIONAL", detail: "not required -- PyTorch CUDA wheels bundle their own runtime", version: null },
    { name: "PyTorch", state: "OPTIONAL", detail: "not installed in this environment", version: null },
  ];
}
